We are now focusing on the development of a bayesian optimization routine. Since we have a tonne of knobs, we have manually selected a few. This will be called optimization routine year0. In this, we will record the state of year0, then take the previous decisions of year0, suggest some values, apply those values, and see year1. We want to optimize for the following - 

Cost function = (performance_summary.unit_sales * 3.5) + 
(income_statement.net_income * 4.52) + 
...
We obviously cannot exceed this total budget - thus, Total Budget = SF_Size + Advertising_Size + Promotion_Size. We have effectively turned 3 knobs into 2, given that the budget is locked. 
